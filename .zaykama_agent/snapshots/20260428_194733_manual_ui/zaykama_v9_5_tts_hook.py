...
class ZaykamaV95TTSHook:
    def __init__(self, export_mode: str = "internal_preview", headless: bool = False, input_path: str | None = None):
    ...
    def load_mongolian_dubbing_memory(self) -> None:
        ...
        if path.exists():
            try:
                ...
                self.log_message(f"Loaded Mongolian dubbing memory with {len(self.dubbing_memory)} entries.")
    ...
    def assemble_dubbed_audio_master(self, segments: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        sample_width = 2
        frames = bytearray()
        for segment in sorted(provider_segments, key=lambda item: float(item.get("start", 0.0))):
            with wave.open(str(segment["ttsPath"]), "rb") as wav_in:
                data = wav_in.readframes(wav_in.getnframes())
                if wav_in.getnchannels() != channels or wav_in.getsampwidth() != sample_width or wav_in.getframerate() != sample_rate:
        ...
        return {"ok": True, "path": output_path, "mode": "provider_tts_assembled", "skipped": False, "reason": "Provider TTS audio master assembled"}
    ...
    def create_final_video(self) -> dict[str, Any]:
        ...
        if not self.input_path or not Path(self.input_path).exists():
            return {"ok": False, "path": None, "skipped": True, "reason": "No input video provided"}
        if not (
            self.real_transcription_used
            and self.real_translation_used
            and self.real_speech_tts_used
            and self.real_audio_master_used
            and self.audio_master_mode == "provider_tts_assembled"
            and self.audio_master_path
            and Path(self.audio_master_path).exists()
            and self.tts_summary.get("provider_tts", 0) > 0
        ):
            return {"ok": False, "path": None, "skipped": True, "reason": "Final video merge disabled: no real assembled dubbed audio master"}
        if not shutil.which("ffmpeg"):
            return {"ok": False, "path": None, "skipped": True, "reason": "ffmpeg not available"}
        try:
            subprocess.check_call(["ffmpeg", "-y", "-i", self.input_path, "-i", self.audio_master_path, "-map", "0:v:0", "-map", "1:a:0", "-c:v", "copy", "-shortest", str(output_path)], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    def auto_dub_process(self) -> None:
        ...
        self.export_wav()
        self.export_mp3()
        self.log_message("Final video merge skipped (validation fallback mode).")
    ...
    def run_self_test(self) -> bool:
        ...
        self.real_speech_tts_used = True
        self.tts_summary["provider_tts"] = 1
        provider_segments = [{"id": 1, "start": 0, "end": 1, "ttsMode": "provider_tts", "ttsPath": provider_wav}]
...
def main() -> None:
    ...
    app = ZaykamaV95TTSHook(export_mode=args.export_mode, headless=args.headless or args.self_test or args.provider_smoke, input_path=args.input)
    if args.self_test:
